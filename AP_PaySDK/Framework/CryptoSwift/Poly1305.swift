...
///  message such that an attacker has a negligible chance of producing a valid tag for an inauthentic message.
 ...
 final class Poly1305: Authenticator {
  ...
  init(key: Array<UInt8>) {
     ...
     func squeeze(h: inout Array<UInt32>) {
        assert(h.count == 17)
        var u: UInt32 = 0
        for j in 0..<16 {
            u = u &+ h[j]
     ...
     func add(h: inout Array<UInt32>, c: Array<UInt32>) {
     ...
     func mulmod(h: inout Array<UInt32>, r: Array<UInt32>) {
     ...
     func freeze(h: inout Array<UInt32>) {
        let horig = h
    ...
    /// the key is partitioned into two parts, called "r" and "s"
        func onetimeauth(message input: Array<UInt8>, key k: Array<UInt8>) -> Array<UInt8> {
        // clamp
