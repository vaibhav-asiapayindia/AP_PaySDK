...
final class GCMModeWorker: BlockModeWorker, FinalizingEncryptModeWorker, FinalizingDecryptModeWorker {
    ...
    let additionalBufferSize: Int
     ...
     let mode: GCM.Mode
     var counter: UInt128
     let eky0: UInt128 // move to GF?
     let h: UInt128

    // Additional authenticated data
     let aad: ArraySlice<UInt8>?
...
// MARK: - Local utils
    ...
    func makeCounter(nonce: Array<UInt8>) -> UInt128 {
...
// bits of its argument as a nonnegative integer with the least significant bit on the right
    func incrementCounter(_ counter: UInt128) -> UInt128 {
    let b = counter.i.b + 1
...
/// The Field GF(2^128)
    ...
    // Not used with incremental calculation.
     static func calculateX(ciphertext: [UInt8], x startx: UInt128, h: UInt128, blockSize: Int) -> UInt128 {
    ...
    // block is expected to be padded with addPadding
     static func calculateX(block ciphertextBlock: Array<UInt8>, x: UInt128, h: UInt128, blockSize: Int) -> UInt128 {
        let k = x ^ UInt128(ciphertextBlock)
        return multiply(k, h)
    }

    // Calculate AAD part, for all blocks
     static func calculateX(aad: [UInt8], x startx: UInt128, h: UInt128, blockSize: Int) -> UInt128 {
    ...
    // Multiplication GF(2^128).
     static func multiply(_ x: UInt128, _ y: UInt128) -> UInt128 {
        var z: UInt128 = 0
        var v = x
