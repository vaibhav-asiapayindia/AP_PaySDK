...
/// Counter with Cipher Block Chaining-Message Authentication Code
 struct CCM: StreamMode {
  ...
  let options: BlockModeOption = [.initializationVectorRequired, .useEncryptToDecrypt]
     ...
     let additionalAuthenticatedData: Array<UInt8>?
     let tagLength: Int
     let messageLength: Int // total message length. need to know in advance
...
class CCMModeWorker: StreamModeWorker, SeekableModeWorker, CounterModeWorker, FinalizingEncryptModeWorker, FinalizingDecryptModeWorker {
    ...
    let additionalBufferSize: Int
     var keystreamPosIdx = 0
     let nonce: Array<UInt8>
     var last_y: ArraySlice<UInt8> = []
     var keystream: Array<UInt8> = []
    // Known Tag used to validate during decryption
     var expectedTag: Array<UInt8>?
  ...
  enum Error: Swift.Error {
    ...
    // For the very first time setup new IV (aka y0) from the block0
     func processControlInformation(nonce: [UInt8], tagLength: Int, hasAssociatedData: Bool) {
    ...
    // CBC is block
     var accumulatedPlaintext: [UInt8] = []
...
// t - octet length of T (MAC length). An element of {4,6,8,10,12,14,16}
    func format(nonce N: [UInt8], Q: UInt32, q: UInt8, t: UInt8, hasAssociatedData: Bool) throws -> [UInt8] {
    var flags0: UInt8 = 0
...
/// Q - octet length of P
/// q - octet length of Q. Maximum length (in octets) of payload. An element of {2,3,4,5,6,7,8}
    func format(counter i: Int, nonce N: [UInt8], q: UInt8) throws -> [UInt8] {
    var flags0: UInt8 = 0
...
// Note: It's similar to ZeroPadding, but it's not the same.
    fileprivate func addPadding(_ bytes: Array<UInt8>, blockSize: Int) -> Array<UInt8> {
    if bytes.isEmpty {
